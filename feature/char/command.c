...
add_path(string p)
{
	...
	path += ({ p });
	/* 保护特权使用者的设计：指令搜寻路径改变时，印出讯息让使用者知道。 */
	if( wizardp(this_object()) )
	    receive(HIR "新增指令搜寻路径：" NOR + p + "\n");
    }
}
...
{
    if( geteuid(previous_object()) != ROOT_UID ) error("Permission denied\n");
...
// cmd_quit()
//
// 这个是用来在万一 quit 指令坏掉时，备用的 quit。
private int
cmd_quit(string arg)
{
    if( !catch(call_other(QUIT_CMD, "main", this_object(), arg)) ) {
	// 若成功，this_object() 已经被 destruct 了，立即返回。
	return 1;
    }
