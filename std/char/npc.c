...
static void heart_beat()
{
    ...
    if( living(this_object()) && clonep(this_object()) && mapp(schedule = query("schedule")) ) {
		mapping gt;
		int time_tag;
...
void die()
{
    ...
    if( objectp(ob = last_damage_giver()) && ob != this_object() && mapp(bounty = query("bounty")) ) {
		string score;
		int amount;
...
int random_move()
{
    ...
    if( environment()->is_area() ) {
    	if( !(dirs = environment()->query_exits(this_object()->query("area_info/x_axis"),
											   this_object()->query("area_info/y_axis")) ) ) return 0;
    }
    else {
...
void setup()
{
    ...
    set_skill("combat", lv*2);
    set_skill("dodge", lv*2);
    set_skill("parry", lv*2);
    set_skill("savage hit", lv*2);
    set_skill("heavy hit", lv*2);
    set_skill("continual hit", lv*2);
    set_skill("force", lv*2);
    set_skill("ninja", lv*2);
    set_skill("magic", lv*2);
