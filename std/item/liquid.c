...
string short(int raw)
{
	...
	if( volume < 500 )
		...
		return "大约" + chinese_number((volume+500)/1000) + "升左右的"
		+ ::short(raw);
}
...
void dissolve_with(object ob)
{
	...
	if( ob->query_volume() <= volume ) {
		...
		set_name("不明液体", ({ "unknown liquid", "liquid"}) );
		set("long", "一些不明成份的液体。\n");
		add_volume(ob->query_volume());
		ob->set_volume(0);
	} else {
		ob->set("density", (float)(weight() + ob->weight())  / (volume + ob->query_volume()) );
		ob->set_name("不明液体", ({ "unknown liquid", "liquid"}) );
		ob->set("long", "一些不明成份的液体。\n");
...
varargs int move(mixed dest, int silent)
{
	...
	if( !dest->query("liquid_container") )
		return notify_fail( name() + "必须放在容器里。\n");
