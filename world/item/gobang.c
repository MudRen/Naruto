...
void create()
{
 set("short", "棋室");
 set("long", @LONG
这个幽静的棋室是专为爱好棋奕的同好开辟的，也是发呆的好地
方。这个房间里有一张石桌，上面画有一张围棋盘。桌角摆放著棋子
。另外还有两张石凳，颜色一白一黑。在门口的墙壁上贴著一张下棋
指南(help)。
  ...
  if(status==WQ_NOT_PLAYING) 
    return notify_fail("还没新开一局棋呐。\n");
        if(status!=WQ_PLAYING_WUZI)
                return notify_fail("目前只提供五子棋的悔棋功\能。\n");
        if(!WQ_Started)
...
{
  if ( me->query_temp("weiqi_seat") )
    return notify_fail("你现在还坐著呢，怎么离开啊？\n");
