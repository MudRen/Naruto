...
void construct_shadow(object me)
{
    ...
    if( me->query_skill(SKILL_NAME, 1) < 40 ) {
        write("你的忍术分身术技能等级不足，每多四十级可多增加一只分身。\n");
        return;
    }
    ...
    message_vision("$N双手快速的结印，施展了"HIW"分身术"NOR"！！\n", me);
    ...
    shadow->set("skill", "alter ego");  // shadow是由alter ego技能叫出
...
int perform_action(object me, string act, object target)
{
    ...
    if( me->is_busy() ) return notify_fail("你现在没有空使用分身术。\n");

    switch( act ) {
        case   "shadow": construct_shadow(me); break;
        case "destruct": destruct_shadow(me);  break;
        default: return notify_fail("目前分身有 shadow, destruct 两种功\能。\n"); break;
    }
